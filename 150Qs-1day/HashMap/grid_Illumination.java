        ...
        for(int i=0; i<queries.length; i++){
            ...
            for(int[] dir: dirs){
                ...
                if(x1>=0 && y1>=0 && x1<n && y1<n && m5.containsKey(x1*n+y1)){
                    ...
                    m1.put(x1, m1.getOrDefault(x1,1) - time);
                    m2.put(y1, m2.getOrDefault(y1,1) - time);
                    m3.put(x1-y1, m3.getOrDefault(x1-y1,1) - time);
                    m4.put(x1+y1, m4.getOrDefault(x1+y1,1) - time);
                    m5.remove(x1*n+y1);
                }
            }
        }
